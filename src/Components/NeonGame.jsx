...
const NeonGame = ({ onBackToMenu }) => {
  ...
  const jump = useCallback(() => {
    const player = gameObjects.current.player;
    if (player.jumpCount < 2 || coyoteTimeRef.current > 0) {
      player.velocityY = -15;
      player.isJumping = true;
      player.jumpCount++;
      coyoteTimeRef.current = 0;
  ...
  const updateGame = useCallback(() => {
    ...
    if (jumpBufferRef.current > 0 && (player.jumpCount < 2 || coyoteTimeRef.current > 0)) {
      jump();
      jumpBufferRef.current = 0;
    }

    // Air control
    if (keysRef.current['ArrowLeft']) player.x -= 4;
    if (keysRef.current['ArrowRight']) player.x += 4;
  ...
  return (
    <div className="neon-game">
      ...
      <div className="game-container">
        ...
        {['menu', 'paused', 'gameOver'].includes(gameState) && (
          <div className="game-overlay">
            <div className="menu-content">
              <h2>{gameState === 'menu' ? 'NEON RUNNER' : gameState === 'paused' ? 'PAUSED' : 'GAME OVER'}</h2>
              {gameState === 'gameOver' && <div className="final-score">Score: {score}</div>}
              {score === highScore && score > 0 && gameState === 'gameOver' && (
                <div className="new-record">🎉 NEW HIGH SCORE! 🎉</div>
              )}
              <div className="controls-info">
                <p><strong>Controls:</strong></p>
              ...
              <button className="play-button" onClick={startGame}>
                <Play size={20} /> {gameState === 'paused' ? 'Resume' : 'Start Game'}
              </button>
              {gameState !== 'menu' && (
                <button className="restart-button" onClick={startGame}>
                  <RotateCcw size={20} /> Restart
      ...
      <div className="game-instructions">
        <p>Master jump, dash, and air movement. Get that high score. 🚀</p>
      </div>
    </div>
  );
};
