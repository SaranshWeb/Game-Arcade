...
const LandingPage = () => {
  const [currentTime, setCurrentTime] = useState(new Date());
  const [currentView, setCurrentView] = useState('landing'); // 'landing', 'neonGame', 'spaceInvaders'
  ...
  const handlePlayGame = (gameName) => {
    if (gameName === 'Neon Runner') {
      setCurrentView('neonGame');
    } else if (gameName === 'Space Invaders Redux') {
      setCurrentView('spaceInvaders');
    } else {
      // For other games, show alert for now
      alert(`${gameName} coming soon! Only Neon Runner and Space Invaders Redux are available right now.`);
    }
  };
  ...
  // If we're in game view, show the appropriate game
  if (currentView === 'neonGame') {
    return <NeonGame onBackToMenu={handleBackToMenu} />;
  }
  
  if (currentView === 'spaceInvaders') {
    return <SpaceInvaders onBackToMenu={handleBackToMenu} />;
  }
